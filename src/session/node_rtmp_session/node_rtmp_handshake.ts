...
function calcHmac(data: any, key: any) {
  let hmac = Crypto.createHmac('sha256', key);
  hmac.update(data);
  return hmac.digest();
}

function GetClientGenuineConstDigestOffset(buf: any) {
  let offset = buf[0] + buf[1] + buf[2] + buf[3];
  offset = (offset % 728) + 12;
  return offset;
}

function GetServerGenuineConstDigestOffset(buf: any) {
  let offset = buf[0] + buf[1] + buf[2] + buf[3];
  offset = (offset % 728) + 776;
  return offset;
}

function detectClientMessageFormat(clientsig: any) {
...
function generateS1(messageFormat: any) {
...
function generateS2(messageFormat: any, clientsig: any, callback?: any) {
...
function generateS0S1S2(clientsig: any) {
